...
(function () {
  ...
  function applyVerificationAndAccountBadge(user) {
    ...
    if (accountStatus && accountStatus !== "active") {
      if (accountStatus === "suspended") {
        setBadge(
          badge,
          "badge",
          `<i class="fas fa-ban"></i> ${t("Account Suspended")}`,
      ...
      if (accountStatus === "deactivated") {
        setBadge(
          badge,
          "badge",
          `<i class="fas fa-user-slash"></i> ${t("Account Deactivated")}`,
      ...
      setBadge(
        badge,
        "badge",
        `<i class="fas fa-info-circle"></i> ${t("Account")}: ${accountStatus}`,
    ...
    if (verificationStatus === "verified") {
      setBadge(
        ...
        `<i class="fas fa-check-circle"></i> ${t("Verified Provider")}`,
      );
      return;
    }
    ...
    if (verificationStatus === "rejected") {
      setBadge(
        badge,
        "badge",
        `<i class="fas fa-times-circle"></i> ${t("Verification Rejected")}`,
        "linear-gradient(135deg, #ef4444, #dc2626)",
      );
      return;
    }
    ...
    setBadge(
      badge,
      "badge badge-pending",
      `<i class="fas fa-clock"></i> ${t("Pending Verification")}`,
    );
  }
  ...
  async function loadAndApply() {
    ...
    const providerName = user.business_name || fullName || t("Provider");

    safeSetText("provider-name", providerName);
    safeSetText("header-user-name", user.first_name || t("Provider"));
