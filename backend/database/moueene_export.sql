...
--
-- Temporary view structure for view `active_bookings_view`
--
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `generate_booking_reference` BEFORE INSERT ON `bookings` FOR EACH ROW BEGIN
...
--
-- Temporary view structure for view `provider_stats_view`
--

DROP TABLE IF EXISTS `provider_stats_view`;
/*!50001 DROP VIEW IF EXISTS `provider_stats_view`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `provider_stats_view` AS SELECT 
 1 AS `provider_id`,
 1 AS `provider_name`,
 1 AS `email`,
 1 AS `phone`,
...
CREATE TABLE `providers` (
  ...
  `country` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT 'Morocco',
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_provider_rating_after_insert` AFTER INSERT ON `reviews` FOR EACH ROW BEGIN
    UPDATE providers
    SET average_rating = (
        SELECT AVG(rating)
        FROM reviews
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_service_rating_after_review` AFTER INSERT ON `reviews` FOR EACH ROW BEGIN
    UPDATE services
    SET average_rating = (
        SELECT AVG(rating)
        FROM reviews
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_provider_rating_after_update` AFTER UPDATE ON `reviews` FOR EACH ROW BEGIN
    UPDATE providers
    SET average_rating = (
        SELECT AVG(rating)
        FROM reviews
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `update_provider_rating_after_delete` AFTER DELETE ON `reviews` FOR EACH ROW BEGIN
    UPDATE providers
...
/*!50001 VIEW `active_bookings_view` AS select `b`.`booking_id` AS `booking_id`,`b`.`booking_reference` AS `booking_reference`,`b`.`booking_date` AS `booking_date`,`b`.`booking_time` AS `booking_time`,`b`.`booking_status` AS `booking_status`,`b`.`payment_status` AS `payment_status`,`b`.`final_price` AS `final_price`,concat(`u`.`first_name`,' ',`u`.`last_name`) AS `customer_name`,`u`.`email` AS `customer_email`,`u`.`phone` AS `customer_phone`,concat(`p`.`first_name`,' ',`p`.`last_name`) AS `provider_name`,`p`.`email` AS `provider_email`,`p`.`phone` AS `provider_phone`,`s`.`service_name` AS `service_name`,`sc`.`category_name` AS `category_name`,`b`.`created_at` AS `created_at` from ((((`bookings` `b` join `users` `u` on((`b`.`user_id` = `u`.`user_id`))) join `providers` `p` on((`b`.`provider_id` = `p`.`provider_id`))) join `services` `s` on((`b`.`service_id` = `s`.`service_id`))) join `service_categories` `sc` on((`s`.`category_id` = `sc`.`category_id`))) where (`b`.`booking_status` in ('pending','confirmed','in_progress')) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `provider_stats_view`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `provider_stats_view` AS select `p`.`provider_id` AS `provider_id`,concat(`p`.`first_name`,' ',`p`.`last_name`) AS `provider_name`,`p`.`email` AS `email`,`p`.`phone` AS `phone`,`p`.`city` AS `city`,`p`.`average_rating` AS `average_rating`,`p`.`total_reviews` AS `total_reviews`,`p`.`total_bookings` AS `total_bookings`,`p`.`completed_bookings` AS `completed_bookings`,`p`.`verification_status` AS `verification_status`,`p`.`account_status` AS `account_status`,count(distinct `ps`.`service_id`) AS `services_offered`,`p`.`created_at` AS `created_at` from (`providers` `p` left join `provider_services` `ps` on((`p`.`provider_id` = `ps`.`provider_id`))) group by `p`.`provider_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-01-28 17:28:41
